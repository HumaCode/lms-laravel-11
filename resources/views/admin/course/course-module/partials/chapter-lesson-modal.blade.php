<div class="modal-content">
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">{{ @$editMode == true ? 'Update Chapter' : 'Create Chapter' }}
        </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
    <form
        action="{{ @$editMode == true ? route('admin.courses-content.update-lesson', @$lesson?->id) : route('admin.courses-content.store-lesson') }}"
        method="POST">
        @csrf
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="storage" id="storage" class="form-control storage">
                            ...
                            @foreach (config('course.video_sources') as $source => $name)
                                <option value="{{ $source }}" @selected(@$lesson?->storage == $source)>{{ $name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="file_type" id="file_type" class="form-control">
                            ...
                            @foreach (config('course.file_types') as $source => $name)
                                <option value="{{ $source }}" @selected(@$lesson?->file_type == $source)>{{ $name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
